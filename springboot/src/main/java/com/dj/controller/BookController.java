...
import com.dj.controller.request.BookPageRequest;
    ...
    public void download(@PathVariable String flag, @RequestParam(required = false) String play, HttpServletResponse response) {
        ...
        try {
            if (StrUtil.isNotEmpty(fileName)) {
                ...
                if ("1".equals(play)) {
                    response.addHeader("Content-Disposition", "inline;filename=" + URLEncoder.encode(realName, "UTF-8"));  //预览
                } else {
                    response.addHeader("Content-Disposition", "attachment;filename=" + URLEncoder.encode(realName, "UTF-8"));  //附件
                }
