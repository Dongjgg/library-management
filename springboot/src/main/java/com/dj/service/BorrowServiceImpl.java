...
public class BorrowServiceImpl implements BorrowService {
    ...
    public void save(Borrow obj) {
        ...
        if (!user.isStatus()){
            throw new ServiceException("该用户已被禁用，查询看详情");
        }
        ...
        Integer score = book.getScore() * obj.getDays(); //score=借一本的积分乘以天数
        if (score>account){
...
//        obj.setId(null);  // 新数据
    ...
    public Map<String, Object> getCountByTimeRange(String timeRange) {
        ...
        // getCountByTimeRange 不会统计数据库没有的日期，比如 数据库 2.4 这一天数据没有，他不会返回 date=2023-2-04,count=0 这个数据
